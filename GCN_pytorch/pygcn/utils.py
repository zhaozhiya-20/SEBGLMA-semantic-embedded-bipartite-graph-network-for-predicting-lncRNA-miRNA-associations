...
def load_data(path="../data/cora/", dataset="cora") -> object:
    ...
    idx_features_labels = np.genfromtxt("{}{}.content".format(path, dataset),
                                        dtype=np.dtype(str))  # genfromtxt就是将txt文件格式转化为numpy格式的数组
    features = sp.csr_matrix(idx_features_labels[:, 1:-1], dtype=np.float32)  # 取特征feature
    labels = encode_onehot(idx_features_labels[:, -1])  # one-hot label(2708*7)
    ...
    adj = normalize(adj + sp.eye(adj.shape[0]))   # 对A+I归一化，就是图卷积层前面的系数简化
    # 训练，验证，测试的样本
    idx_train = range(140)
