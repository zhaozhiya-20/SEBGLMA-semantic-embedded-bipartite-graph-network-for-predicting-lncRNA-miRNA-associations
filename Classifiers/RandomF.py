...
rfc = RandomForestClassifier(n_estimators=80,  oob_score=True, random_state=42)
...
tprs = []
fpr, tpr, threshold = roc_curve(Y_test[0], Real_data1[0])
tprs.append(interp(mean_fpr, fpr, tpr))
fpr1, tpr1, threshold1 = roc_curve(Y_test[1], Real_data1[1])
tprs.append(interp(mean_fpr, fpr1, tpr1))
fpr2, tpr2, threshold2 = roc_curve(Y_test[2], Real_data1[2])
tprs.append(interp(mean_fpr, fpr2, tpr2))
fpr3, tpr3, threshold3 = roc_curve(Y_test[3], Real_data1[3])
tprs.append(interp(mean_fpr, fpr3, tpr3))
fpr4, tpr4, threshold4 = roc_curve(Y_test[4], Real_data1[4])
...
plt.show()
fig.savefig(r'E:\论文\LncRNA-miRNA\result\RaF\Raf_ROC.tif', dpi=600, format='tif')
np.savetxt(r"E:\论文\LncRNA-miRNA\result\RaF\mean_Fpr.csv", fpr, delimiter=",")
np.savetxt(r"E:\论文\LncRNA-miRNA\result\RaF\\mean_Tpr.csv", tpr, delimiter=",")
mean_recall = np.linspace(1, 0, 100)  # Linear interpolation method
precisions = []
# Prepare for PR curve
precision1, recall1, thresholds = precision_recall_curve(Y_test[0], Real_data1[0], pos_label=None,
                                                         sample_weight=None)
...
precision2, recall2, thresholds2 = precision_recall_curve(Y_test[1], Real_data1[1], pos_label=None,
                                                          sample_weight=None)
...
precision3, recall3, thresholds3 = precision_recall_curve(Y_test[2], Real_data1[2], pos_label=None,
                                                          sample_weight=None)
...
precision4, recall4, thresholds4 = precision_recall_curve(Y_test[3], Real_data1[3], pos_label=None,
                                                          sample_weight=None)
...
precision5, recall5, thresholds5 = precision_recall_curve(Y_test[4], Real_data1[4], pos_label=None,
                                                          sample_weight=None)
...
plt.show()
fig2.savefig(r'E:\论文\LncRNA-miRNA\result\RaF\RaF_PR.tif', dpi=600, format='tif')
...
ax.spines['top'].set_linewidth(2)  ###设置上部坐标轴的粗细
plt.show()
np.savetxt(r"E:\论文\LncRNA-miRNA\result\RaF\mean_recall.csv", mean_recall, delimiter=",")
np.savetxt(r"E:\论文\LncRNA-miRNA\result\RaF\mean_precision.csv", mean_precision, delimiter=',')